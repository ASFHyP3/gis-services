...
def enable_wms(service_draft: str):
    sd_tree = etree.parse(service_draft)
    extensions = sd_tree.find("/Configurations/SVCConfiguration/Definition/Extensions")
    ...
    for sub_el in svc_ext_elements:
        if sub_el[1] == '':
            ...
            sub_el_path = sd_tree.find(f"/Configurations/SVCConfiguration/Definition/Extensions/SVCExtension/{sub_el[0]}")
            etree.SubElement(sub_el_path, 'PropertyArray', {xsi_type: 'typens:ArrayOfPropertySetProperty'})
    sd_tree.find("/Configurations/SVCConfiguration/Definition/Extensions/SVCExtension/Enabled").text = 'true'
    sub_props = sd_tree.find("/Configurations/SVCConfiguration/Definition/Extensions/SVCExtension/Info/PropertyArray")
    for el in [('WebEnabled', 'true'),
        ...
        se_key = etree.SubElement(add_prop, 'Key')
        se_key.text = el[0]
        se_value = etree.SubElement(add_prop, 'Value', {xsi_type: 'xs:string'})
        se_value.text = el[1]
    sub_props = sd_tree.find("/Configurations/SVCConfiguration/Definition/Extensions/SVCExtension/Props/PropertyArray")
    ...
    sd_tree.find("/Configurations/SVCConfiguration/Definition/Extensions/SVCExtension/TypeName").text = 'WMSServer'
    sd_tree.write(service_draft)
...
try:
    ...
    for attempt in Retrying(stop=stop_after_attempt(3), wait=wait_fixed(60), reraise=True,
        ...
        with attempt:
            ...
            service_draft = os.path.join(args.working_directory, f'{output_name}.sddraft')
    ...
    logging.info(f'Creating draft service definition {service_draft}')
    arcpy.CreateImageSDDraft(
        raster_or_mosaic_layer=mosaic_dataset,
        out_sddraft=service_draft,
        service_name=config['service_name'],
    )

    logging.info(f'Enabling WMS in {service_draft}')
    enable_wms(service_draft)
