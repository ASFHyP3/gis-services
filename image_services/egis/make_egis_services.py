...
def get_raster_metadata(raster_path: str) -> dict:
    ...
    download_url = (
        f"https://asf-ngap2w-p-s1-global-coherence.s3.us-west-2.amazonaws.com/{key}"
    )
...
def calculate_overview_fields(mosaic_dataset, local_path):
    ...
    if ds_cursor is not None:
        ...
        for row in ds_cursor:
            if row[0] == "Dataset":
                _, ProdTypeOvField, PolOvField, SeasonOvCode, _, _, _ = row[6].split(
                    "_"
                )
                ...
                try:
                    season = next(
                        ...
                        if SEASONS[season]["SeasonCode"] == SeasonOvCode
                    )
                ...
                row[5] = (
                    f"{ProdTypeOvField}_{PolOvField}_{SeasonOvCode} Mosaic Overview"
                )
