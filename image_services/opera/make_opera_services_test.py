...
def update_csv(csv_file: str, rasters: List[str], bucket: str):
    ...
    if new_rasters:
        ...
        with open(csv_file, "a", newline="") as csvfile:
            writer = csv.DictWriter(
                csvfile, fieldnames=header_record.keys(), lineterminator=os.linesep
            )
    ...
    with open(csv_file, "w", newline="") as csvfile:
        writer = csv.DictWriter(
            csvfile, fieldnames=records[0].keys(), lineterminator=os.linesep
        )
        writer.writeheader()
...
def calculate_overview_fields(mosaic_dataset, local_path):
    ...
    start_dates = [
        row[1]
        for row in arcpy.da.SearchCursor(mosaic_dataset, ["Tag", "StartDate"])
        if row[0] != "Dataset"
    ]
    overview_start_date = min(start_dates).replace(microsecond=0) + datetime.timedelta(
        hours=-8
    )
    overview_end_date = max(start_dates).replace(microsecond=0) + datetime.timedelta(
        hours=8
    )

    tag = [
        row[0]
        for row in arcpy.da.SearchCursor(mosaic_dataset, ["Tag"])
        if row[0] != "Dataset"
    ]
...
def add_property(
    property_set: etree.Element, property_key: str, property_value: str
) -> None:
    xsi_type = etree.QName("http://www.w3.org/2001/XMLSchema-instance", "type")
    prop = etree.SubElement(
        property_set, "PropertySetProperty", {xsi_type: "typens:PropertySetProperty"}
    )
...
def build_wms_extension() -> etree.Element:
    ...
    property_set = etree.SubElement(
        svc_extension, "Props", {xsi_type: "typens:PropertySet"}
    )
    property_array = etree.SubElement(
        property_set, "PropertyArray", {xsi_type: "typens:ArrayOfPropertySetProperty"}
    )
    ...
    info_property_set = etree.SubElement(
        svc_extension, "Info", {xsi_type: "typens:PropertySet"}
    )
    info_property_array = etree.SubElement(
        info_property_set,
        "PropertyArray",
...
def main():
    ...
    parser.add_argument(
        "--server-connection-file", default="/home/arcgis/server_connection.json"
    )
    ...
    csv_file = os.path.join(
        args.working_directory, f'{config["project_name"]}_{config["dataset_name"]}.csv'
    )

    raster_function_template = "".join(
        [
            f"{template_directory / template};"
            for template in config["raster_function_templates"]
        ]
    )
    if config["default_raster_function_template"] != "None":
        default_raster_function_template = str(
            template_directory / config["default_raster_function_template"]
        )
    else:
    ...
    try:
        rasters = get_rasters(
            config["bucket"], config["s3_prefix"], config["s3_suffix"]
        )
        ...
        for attempt in Retrying(
            ...
            with attempt:
                ...
                service_definition = os.path.join(
                    args.working_directory, f"{output_name}.sd"
                )
        ...
        with tempfile.NamedTemporaryFile(suffix=".sddraft") as service_definition_draft:
            logging.info(
                f"Creating draft service definition {service_definition_draft.name}"
            )
            ...
            extensions = tree.find(
                "/Configurations/SVCConfiguration/Definition/Extensions"
            )
            extensions.append(build_wms_extension())

            logging.info(
                f"Editing service definition overrides for {service_definition_draft.name}"
            )
